...
```{r identify_unknown_authors}
identified_commits <- standardized_commits %>% 
  ...
  mutate(author_login = if_else(is.na(author_login), standardized_author_name, author_login))
author_ids <- identified_commits %>% 
  select(author_login, standardized_author_name, commit_author_email) %>% 
  unique()
```

```{r clean,message=TRUE}
cleaned_commits <- identified_commits %>%
